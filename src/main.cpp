      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			// Adapted from Walkthrough part 1: Create a list of widely spaced (x,y) waypoints evenly spaced at 30 m
			// Later on we will interpolate these waypoints with a spline (could try to fit a polynomial) 
			// and fill it with more points in order to control the speed.

			vector <double> ptsx;
			vector <double> ptsy;

			// reference state (either reference to starting point or to previous path end points)
			double ref_x = car_x;
			double ref_y = car_y;
			...
			{
				//Reference state is redefined as previous path end point
			...
			// In Frenet coordinates add 30 m spaced points ahead of the starting reference
			...
			{
				//shift reference angle to 0 degrees (for referencing to car's coordinates, similar to MPC project)
			...
			//Calculate how to break up spline points so that we travel at the objective velocity
			...
			//Fill up the rest of the path planner after filling it up with previous points
			//for now only 50 points will be added

			for (int i = 0; i < 50-previous_path_x.size(); i++) 
			{
				double N = (target_dist / (0.02 * ref_vel / 2.24)); //2.24 is for the conversion to m/s (why do we keep using imperial units??)
				...
				//rotate back to world's reference frame
